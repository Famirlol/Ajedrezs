/*
    Este archivo contiene funciones miembro del archivo chessGame.h
*/
...
: board(bordCol1,bordCol2)
{
    ...
    font.loadFromFile("C:/Users/User/Documents/Chess-SFML-master/Chess-SFML-master/src/Textures/arial.ttf");
...
void ChessGame::updateInfo(){
    textTurn.setString("Turno: " + std::to_string(turn));
    textLastMove.setString(lastMove);

    if(!mate){
        if(playerTurn)
            textSituation.setString("Turno de las blancas");
        else
            textSituation.setString("Turno de las negras");
...
bool ChessGame::selectPiece(int pos){
    ...
    for(int i=0; i<16; i++){
        if(playerTurn){ // Si es el turno de las blancas
...
void ChessGame::moveSelected(int pos){
    ...
    // Checa las posiciones de las piezas para ver posibles movimientos
    for(int i=0;i<selectedPiece->getPossibleMoves().size();i++){
    ...
    if(validMove){
        ...
        lastMove = "Ultimo turno:\n" + selectedPiece->toString();
        for(int i=0; i<16; i++){
            if(selectedPiece->getPlayer()){ // Si es blanco
                if(blackPieces[i].getPosition() == pos){
        ...
        playerTurn = !playerTurn; // Aca cambia el turno de los jugadores
        calcPossibleMoves();
    }
...
void ChessGame::calcPossibleMoves(){
    ...
    for(int x=0; x<32; x++){
        ...
        // Calcula los movimientos de tmPieza por pieza para calcular los tipos de movimientos
    ...
    // Elimina movimientos ilegales de las piezas de los jugadores
    for(int x = 0; x < 16; x++){
        if(playerTurn){
